...
class BudgetListTests(APITestCase):
    ...
    def _create_expense(self, budgetlist_pk, budget_pk):
        url = reverse("expense-list", kwargs={"budgetlist_pk": budgetlist_pk, "budget_pk": budget_pk})
        data = {
            "name": "expense-name",
            "amount": 1.00,
    ...
    def _create_income(self, budgetlist_pk, budget_pk):
        url = reverse("income-list", kwargs={"budgetlist_pk": budgetlist_pk, "budget_pk": budget_pk})
        data = {
            "name": "income-name",
            "amount": 1.00,
            "category": "test_category"
    ...
    def test_create_expense(self):
        ...
        response = self._create_expense(budget_list_response.data["id"], budget_response.data["id"])
    ...
    def test_create_income(self):
        budget_list_response = self._create_budget_list()
        budget_response = self._create_budget(budget_list_response.data["id"])
        response = self._create_income(budget_list_response.data["id"], budget_response.data["id"])

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_add_participant(self):
        new_user = User.objects.create_user(username="newuser", password="newuserpass123")
